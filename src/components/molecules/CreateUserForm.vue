...
export default {
  ...
  methods: {
    ...
    post_data: function (){
      ...
      if (validate_result){
        axios.post('/user_conf/new/', {"user_name": this.ID, "password": this.pw_1}).then((response) => {
          this.$store.dispatch('save_token_info_permanent', {"user_name": this.ID, "token": response.data['token'], "contain_chuni_user": false})
          return true
        })
            .catch((error) => {
              var text= ""
